...
tpm.sample_zshift = tpm.sample_zshift_gt + torch.linspace(-5e-6, 5e-6, N_sz).view(*((-1,) + (1,)*3))
tpm.obj2_zshift = tpm.obj2_zshift_gt + torch.linspace(-20e-6, 20e-6, N_oz).view(*((-1,) + (1,)*4))
tpm.sample.shell_thickness_m = torch.linspace(115e-6, 155e-6, N_ir).view(*((-1,) + (1,)*5))
tpm.sample.outer_radius_m = torch.linspace(460e-6, 510e-6, N_or).view(*((-1,) + (1,)*6))
...
plt.imshow(errorslice, origin='lower',
    extent=(tpm.sample.shell_thickness_m.min()*1e3, tpm.sample.shell_thickness_m.max()*1e3, tpm.sample.outer_radius_m.min()*1e3, tpm.sample.outer_radius_m.max()*1e3))
plt.colorbar()

plt.contour(tpm.sample.shell_thickness_m.squeeze()*1e3, tpm.sample.outer_radius_m.squeeze()*1e3, errorslice, levels=25, colors='k')
