...
tiffolder = fullfile(dirs.expdata, '/raylearn-data/TPM/TPM-3D-scans/230908_beads_testshift/');

tiff_flat = load_tiff([tiffolder 'beads-zoom12-zstep0.250um-no-correction_00001.tif'], calibration_data, '', true); disp('Loaded flat')
tiff_xshift = load_tiff([tiffolder 'beads-zoom12-zstep0.250um-xshift_00001.tif'], calibration_data, '', true); disp('Loaded x-shift')
tiff_yshift = load_tiff([tiffolder 'beads-zoom12-zstep0.250um-yshift_00001.tif'], calibration_data, '', true); disp('Loaded y-shift')
...
xshift_in_um = mean(xshifts_in_um, 1);
yshift_in_um = mean(yshifts_in_um, 1);
...
clear tiff_xshift
clear tiff_yshift
tiff_zshift_with_obj = load_tiff([tiffolder 'beads-zoom12-zstep0.250um-zshift_with_obj_zshift_00001.tif'], calibration_data, '', true);  disp('Loaded z-shift')
thresh = 2000;
[zshift_in_um, zshift_index, shift_products, i_shifts] = compute_zshift_in_um(bg_median, tiff_flat, tiff_zshift_with_obj, thresh);
figure;

%% Plot z
plot(tiff_flat.zstep_um .* (i_shifts - floor((size(shift_products, 2)) / 2)), fftshift(shift_products), '.-')
...
stackflat_shifted = circshift(stackflat, [0 0 -5]);         % Fill in slice shift for z-shift match

defocus_xyshifts_in_um = [];

slice_start = 5+1;
slice_range = slice_start:size(tiff_flat.framestack_raw, 3)-5;
starttime = now;
count = 1;
for i_zslice = slice_range
    index = i_zslice - slice_start + 1;
...
defocus_xyshift_in_um = mean(defocus_xyshifts_in_um, 1);
...
function [zshift_in_um, zshift_index, shift_products, i_shifts] = compute_zshift_in_um(bg_median, tiff_flat, tiff_shift, thresh)
    stackflat = tiff_flat.framestack_raw - bg_median;
    ...
    i_shifts = 0:(Nz-1);
    starttime = now;
    for i_shift = i_shifts
        stackflat_shifted = circshift(stackflat, [0 0 i_shift]);
        shift_products(i_shift+1) = mean(stackflat_shifted.*stackshift, [1 2 3]);
        eta(i_shift+1, Nz, starttime, 'cmd', 'Computing shifted inner products', 0);
    end
