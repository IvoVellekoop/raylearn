function [rects, N] = BlockedCircleSegments(Nd, D, cx, cy, vis, relativesegmentsize)
...
% relativesegmentsize)
% Input arguments D, cx, cy, vis and
% relativesegmentsize are optional. Defaults are given below.
...
%       Default: 0
% relativesegmentsize: Size of the actual segments, relative to
%       the size when the segments are touching. Default: 1.
%
% === Output: ===
...
narginchk(1,6)      % Check if 1 <= nargin <= 6
...
if nargin < 6       % Segment size relative to
    relativesegmentsize = 1;
end
...
validateattributes(relativesegmentsize, {'numeric'}, {'size', [1 1]})
...
couter = R - R/Nd;  % Distance from center to outer segment center

% Construct x and y coordinates on grid
x  = linspace(cx-couter, cx+couter, Nd);
y  = linspace(cy-couter, cy+couter, Nd);
[X, Y] = meshgrid(x, y);
...
N = length(rcx);
rw = D/Nd * relativesegmentsize * ones(N,1);
rh = rw;
